...
def verify_edits(request, audit_type='tree'):
    ...
    plots = Plot.history.filter(present=True).filter(_audit_user_rep__lt=10000).filter(_audit_change_type__exact='U').exclude(_audit_diff__exact='').filter(_audit_verified__exact=0)
    treeactions = []
